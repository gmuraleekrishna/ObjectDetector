...
def print_boxes_original(results, dataset, im_num, inp_dim=(416,416)):
	img_name = dataset.annotations[im_num]['name']
	initial_w, initial_h = dataset.size
	inp_w, inp_h = inp_dim
	
	scale = min(inp_w / initial_w, inp_h / initial_h)

	new_w = int(initial_w * scale)
	new_h = int(initial_h * scale)

	h_shift = (inp_h - new_h) // 2
	w_shift = (inp_w - new_w) // 2

	if torch.is_tensor(results):
		results = torch.Tensor.cpu(results).detach().numpy()
	...
	# Shifting Bounding Box coordinates in accordance with gray padding
	results[:, 0] = results[:, 0] - w_shift
	results[:, 1] = results[:, 1] - h_shift
	results[:, 2] = results[:, 2] - w_shift
	results[:, 3] = results[:, 3] - h_shift

	# Scaling Bounding box coordinates according to initial image
	results[:, 0:4] = results[:, 0:4] // scale
	
	image = Image.open(os.path.join(dataset.root, dataset.image_folder, img_name))
	...
	for result in results:
		...
		text = "{0}".format(dataset.class_names[result[-1]])
